    ...
    public function getGenerelIncome($from, $to, $currency = null, $mode)
    {
      ...
      if ($mode == 'detail') {
        foreach ($data as $key => $value) {
          if (!isset($incomeStat[$value->income_name][$value->edate])) {
            $incomeStat[$value->income_name][$value->edate] = 0;
          }
      ...
      if ($mode == 'summery') {
        foreach ($data as $key => $value) {
          if (!isset($incomeStat[$value->income_name])) {
            $incomeStat[$value->income_name] = 0;
          }
